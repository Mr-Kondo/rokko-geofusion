...
By default an open-weights model (Qwen3-VL) runs locally on the GPU, sized to
the memory detected; the first run downloads it. Set vlm.provider to anthropic
or openai (and export the API key) to use a hosted model instead.
...
def main(argv: list[str] | None = None) -> int:
    ...
    result_path = config.paths.reports / "vlm_analysis.json"
    if not args.render_only:
        # Replace the previous result before anything can fail, so that no
        # outcome -- not even an unanticipated exception -- leaves an earlier
        # run's analysis standing as this run's.
        write_json(result_path, {"status": "incomplete",
                                 "reason": "the VLM run started but did not finish"})
